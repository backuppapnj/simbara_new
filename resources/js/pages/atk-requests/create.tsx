...
export default function AtkRequestsCreate({ items, departments, user_department_id }: CreateProps) {
    ...
    const handleAddItem = () => {
        ...
        if (requestItems.some(ri => ri.item_id === selectedItemId)) {
            setStockWarning('Item sudah ditambahkan. Silakan hapus terlebih dahulu jika ingin mengubah jumlah.');
            return;
        }

        // Check stock
        if (quantity > item.stok) {
            setStockWarning(`Stok tidak cukup! Tersedia: ${item.stok} ${item.satuan}`);
            return;
        }
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-y-auto p-4 md:p-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold">Buat Permintaan ATK</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid gap-4 md:grid-cols-2">
                                <div className="space-y-2">
                                    ...
                                    {form.errors.department_id && (
                                        <p className="text-sm text-red-600">{form.errors.department_id}</p>
                                    )}
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    />
                                    {form.errors.tanggal && (
                                        <p className="text-sm text-red-600">{form.errors.tanggal}</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                {form.errors.keterangan && (
                                    <p className="text-sm text-red-600">{form.errors.keterangan}</p>
                                )}
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    {requestItems.length > 0 && (
                        <Card>
                            <CardHeader>
                                <CardTitle>Daftar Barang ({requestItems.length})</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="space-y-2">
                                            ...
                                            >
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-sm text-muted-foreground">
                                                        {item.kode_barang} • Stok: {item.stok} {item.satuan}
                                                    </p>
                                                </div>
                                                <div className="flex items-center gap-4">
                                                    <div className="text-right">
                                                        <p className="font-medium">
                                                            {requestItem.jumlah_diminta} {item.satuan}
                                                        </p>
                                                    ...
                                                    <Button
                                                        ...
                                                        variant="ghost"
                                                        size="icon"
                                                        onClick={() => handleRemoveItem(requestItem.item_id)}
                                                    >
                    ...
                    <div className="flex items-center justify-end gap-4">
                        ...
                        <Button
                            type="submit"
                            disabled={form.processing || requestItems.length === 0}
                        >
                            {form.processing ? 'Memproses...' : 'Simpan Permintaan'}
                        </Button>
                    </div>
                </form>
            </div>
